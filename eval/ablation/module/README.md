...
每个变体 A 训 150 epoch + B fine-tune 50 epoch（沿用 main 同 schedule，只换 config）。
...
| `aggregate.py` | 聚合所有 eval JSON → Tab 6 csv + markdown |

## Trainer 侧的依赖

我在 `train/trainer.py` 加了两个 CLI flag（已 commit 到本地）：
...
`make_config.py` 会把 `--no-physics` 等 flag 写到 `trainer_flags.txt`，bash 脚本读取并加到 torchrun。

## 在 8×H100 上的执行流程

```bash
# 0) sync 整个 eval/ablation/module/ 到 /workspace/CAP/eval/ablation/module/
#    并 sync trainer.py 改动
...
# 2) B fine-tune (~6 × 30 min ≈ 3 h)
bash eval/ablation/module/finetune_b.sh

# 3) 跑全套 eval (~6 × 8 min ≈ 1 h，含 main 对照)
...
|---|---|---|---|
| Train A (150 ep) | ~5 h | 30 h | **30 h** |
| Fine-tune B (50 ep) | ~30 min | 3 h | **3 h** |
| Eval (full suite × A+B) | ~10 min | 1 h | **1 h** |
| **TOTAL** | | | **~34 GPU·h ≈ 1.5 wall-clock 天** |

并行说一下：6 个 A train 互不依赖，可以**多机并行**或**1×H100/seed 跑 6 个**（batch 减小到 2-4）。但 sequential 更稳，1.5 天等得起。
...
例：加一个 `"k512_no_hier"` 双消融变体（同时 K=512 和无层级——其实就是 no_hier 因为 K 默认 512），直接在 `variants.py` 里加一项即可。
...
- **Appendix A / B**：每个变体的 train log + eval JSON 存档
- **Discussion**：从 Tab 6 哪几行差距最大反推贡献最大的设计点（典型情况：no_algebraic 应该在 Δ_clos 上爆炸）
